...
runTests() {
  ...
  if [ -f "pubspec.yaml" ] && [ -d "test" ]; then
    ...
    # fvm flutter packages get > /dev/null
    ...
    if grep fvm flutter pubspec.yaml >/dev/null; then
      if [ "$3" == true ]; then
        fvm flutter test || error=true
      else
        ...
        if [ -d "coverage" ]; then
          ...
          fvm flutter test --machine > test-results.json
...
runAnalyze() {
  ...
  if [ -f "pubspec.yaml" ]; then
    ...
    fvm flutter analyze || error=true
  fi
...
runTranslate() {
  ...
  if [ -f "pubspec.yaml" ]; then
    ...
    fvm flutter gen-l10n --arb-dir=lib/core/l10n/ --template-arb-file=intl_pt.arb --output-localization-file=translations.dart --output-class=Tr --output-dir=lib/l10n --no-synthetic-package
  fi
...
runBuildAndroid() {
  ...
  if [ -f "pubspec.yaml" ]; then
    if [[ "$1" = "prod" ]]; then
      echo "Building prod..."
      fvm flutter pub run ../.tools/generate_envs.dart
...
runBuildIOS() {
  ...
  if [ -f "pubspec.yaml" ]; then
    ...
    if [[ $1 = "dev" ]]; then
      echo "Building dev..."
      fvm flutter pub run ../.tools/generate_envs.dart
      fvm flutter build ios --release --flavor dev -t lib/main_dev.dart
    fi
  fi
...
runStorybookBuild() {
  ...
  fvm flutter build web --release || error=true
